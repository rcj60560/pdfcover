...
处理顺序：优先使用视频字幕轨；没有字幕轨时下载临时音频，用 `faster-whisper small.en` 生成英文时间轴，再用 Google Translate 补中文。最终只留下 `.md` 和 `.xlsx`，临时音频自动删除。机器识别/翻译会写进 Markdown 的生成说明，不冒充作者字幕。
...
- 一键模式使用 Google Translate 补齐缺失语言，可能受网络限流影响，结果属于机器翻译，需要抽查。
