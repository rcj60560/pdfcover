...
## 待优化需求清单（2026-09-01 暂存）

用户提出「还有地方需要优化」，具体条目待其处理完手头 bug 后补充；当前已知的优化点如下：

| # | 优化点 | 现状 | 建议方案 |
|---|---|---|---|
| 1 | 翻译质量：MyMemory 习语直译（over the moon→飞越月球、table→桌子） | Google 不可达时自动用 MyMemory，可用但生硬 | 设 `HTTPS_PROXY` 走回 Google；或注册 `MYMEMORY_EMAIL` 提额；后续可考虑接入 LLM 翻译 |
| 2 | 网页模式（8500/8600 面板入口）与一键脚本两个入口割裂 | 网页遇"无字幕轨"只报错，不会自动走 Whisper | 网页模式加"识别并生成"按钮，后台调用 direct_generate 同一管线 |
| 3 | Excel 缺"生成方法"元数据行 | MD 底部有生成说明，Excel 只有标题+来源 | 表头第 2 行后加一行生成方法，或加到 docProps |
| 4 | 未登录时无法区分"无字幕轨"与"登录后才展示" | 需要用户手动加 `--browser edge` 重试 | 检测到 412/无轨时提示一键命令加 `--browser` 参数 |
| 5 | 翻译中途失败会丢失已完成 Whisper 转写（整轮重来 ~8 分钟） | 转写结果只在内存 | 转写完成后先落盘 `whisper-transcript.json` 到输出目录，重跑时复用 |
| 6 | （待用户补充） | — | — |
